...
from config import (BANNED_USERS, SOUNCLOUD_IMG_URL, STREAM_IMG_URL,
                    ...
                    adminlist)
from KNMusic import YouTube, app
from KNMusic.core.call import KN
from KNMusic.misc import SUDOERS, db
from KNMusic.utils.database import (is_active_chat, is_music_playing, is_muted,
                                    is_nonadmin_chat, music_off, music_on,
                                    mute_off, mute_on, set_loop)
from KNMusic.utils.decorators.language import languageCB
from KNMusic.utils.formatters import seconds_to_min
from KNMusic.utils.inline.play import stream_markup, telegram_markup
from KNMusic.utils.stream.autoclear import auto_clean
from KNMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await KN.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await KN.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                await CallbackQuery.edit_message_text(
                    f"stream di lewati oleh {mention} !"
                )
                await CallbackQuery.message.reply_text(
                    _["admin_10"].format(mention), disable_web_page_preview=True
                )
                try:
                    return await KN.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await KN.skip_stream(chat_id, link, video=status)
            except Exception:
            ...
            try:
                await KN.skip_stream(chat_id, file_path, video=status)
            except Exception:
                return await mystic.edit_text(_["call_7"])
            ...
            try:
                await KN.skip_stream(chat_id, videoid, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(_["call_7"])
            button = telegram_markup(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
            ...
            try:
                await KN.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(_["call_7"])
            if videoid == "telegram":
                button = telegram_markup(_, chat_id)
